---
description: "Standing rule for fallow: the task-command map (which fallow command to run before a given edit/commit/PR action) and the local pre-commit/pre-push audit gate."
---
...
For non-skill agents, treat the task map above as the local onboarding source: run the listed fallow command before destructive edits, before commits, and before pull request handoff.
